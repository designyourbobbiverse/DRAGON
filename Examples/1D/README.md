...
| Example |  MHD | Limiter | Left state | Both states | Right state |
| --- | --- | --- | --- | --- |--- |
| Sod | NO | MC | $\rho = 1.0$, $p = 1.0$| $v = 0$ | $\rho = 0.125$, $p = 0.1$ |
| Lax | NO | MC | $\rho = 0.445$, $v_x = 0.698$ , $p = 3.528$| $v_y = v_z = 0$ |$\rho = 0.5$, $v_x = 0$, $p = 0.571$ |
...
The Riemann Solver was HLLC for pure hydrodynamics and HLLD for MHD. Most examples use the MC limiter, though the last two were instead run with MINMOD since MC produced spurious oscillations near strong shocks in those.
