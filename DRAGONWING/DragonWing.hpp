...
//Multithreading is done via the ThreadPool class. Important functions:
    ...
    //void* advectParallel(Grid* grid, double dt); //Creates a thread which calls grid->advance_step(dt);
    //void* sourceParallel(SourceTerm* src, Grid* grid, double dt); //Creates a thread which calls src->advance(*grid, dt);
