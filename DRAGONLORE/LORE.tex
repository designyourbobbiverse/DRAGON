...
\cpp{DRAGONWING::ThreadPool} is used by \cpp{DistGrid} (see \Cref{sec:decomposition}) to advance sibling subgrids concurrently, each on its own thread. It is constructed with a fixed thread budget \cpp{N} (one thread per child subgrid); \cpp{launchParallel(grid, dt)} is then called once per child to start \cpp{grid->advance(dt, false)}\footnote{\cpp{grid->advance(dt, false)} bypasses the CFL checks of \Cref{sec:CFL} since these are handled by the parent.} running on a new thread. 
...
After all subgrids are launched via \cpp{launchParallel(grid, dt)}, the parent grid  should call \cpp{waitForCompletion()} on the same \cpp{ThreadPool} object.
