...
The 1D examples (\bash{Examples/1D/}) are classic shock-tube and wave-interaction Riemann problems with well-established reference solutions: Sod, Lax, Shu \& Osher, and the Strong Shock test for pure hydrodynamics; and Brio \& Wu, Dai \& Woodward, and Ryu \& Jones for MHD. Each is a self-contained \bash{Config.h}/\bash{Problem.cpp} pair (plus, for Brio \& Wu, a \bash{Constants.h} setting $\gamma = 2$). All of these problems use outflow boundaries, HLLC (hydro) or HLLD (MHD) as the Riemann solver, and MC limiting for hydro / MINMOD for MHD -- MC was found to produce spurious oscillations near strong shocks in the MHD cases. The specific inputs  for each test are listed in \Cref{tab:1D}. \Cref{fig:1d-hydro} shows DRAGON's output for the hydrodynamic problems, generated with DRAGONGAZE (\Cref{ch:gaze}); the corresponding plots for the MHD problems can be found in Appendix \ref{ch:additional-figures}. \Cref{tab:1d-examples-ic} lists the initial left, right, and shared states set up in each problem's \bash{Problem.cpp}.
