...
The 1D examples (\bash{Examples/1D/}) are classic shock-tube and wave-interaction Riemann problems with well-established reference solutions: Sod, Lax, Shu \& Osher, and the Strong Shock test for pure hydrodynamics; and Brio \& Wu, Dai \& Woodward, and Ryu \& Jones for MHD. Each is a self-contained \bash{Config.h}/\bash{Problem.cpp} pair (plus, for Brio \& Wu, a \bash{Constants.h} setting $\gamma = 2$). All of these problems use outflow boundaries, HLLC (hydro) or HLLD (MHD) as the Riemann solver, and MC limiting for hydro / MINMOD for MHD -- MC was found to produce spurious oscillations near strong shocks in the MHD cases. \Cref{fig:1d-hydro} shows DRAGON's output for the hydrodynamic problems, generated with DRAGONGAZE (\Cref{ch:gaze}); the corresponding plots for the MHD problems can be found in Appendix \ref{ch:additional-figures}.
...
The Kelvin--Helmholtz example sets up a shear layer: a denser right moving band ($\rho_1=2$, $v_x = +V_0$) separated from a lighter left-moving ambient medium ($\rho_2=1, v_x = -V_0$) by two $\tanh$-smoothed interfaces. 
...
The perturbation itself is periodic in $x$ (integer \cpp{kmode=2}), so the seeded pattern tiles smoothly even though the outflow boundary condition doesn't enforce it.
...
\caption{$t=2.0$}
\end{subfigure}
\caption{Density, showing the growth of the Kelvin--Helmholtz instability.}
\label{fig:kh}
\end{figure}
...
The field loop advection test carries a weak magnetic field loop across a periodic domain at uniform velocity. 
...
The MHD linear wave test (cite: GS08) seeds a single, exactly linear eigenmode of the MHD equations on top of a uniform background state and lets it propagate. 
...
\end{subfigure}
\caption{MHD Blast test at $t=0.02$, 2D and 3D.}
\label{fig:mhd-blast}
\end{figure}

The MHD Blast test is the magnetized counterpart to the hydrodynamic Sedov Blast (\Cref{sec:sedov-blast}): a small central region of radius $r_0$ is set to a much higher pressure ($p_\mathrm{blast}=100$) than the surrounding ambient medium ($p_\mathrm{amb}=1$), with uniform density and zero velocity everywhere. 
...
The MHD Rotor test embeds a dense, rigidly rotating disk ($\rho_0=10$, angular velocity $\omega=20$, radius $r_0=0.1$) in a lighter, stationary ambient medium ($\rho_\mathrm{amb}=1$), with a linear taper in density and velocity between $r_0$ and $r_1=0.115$ so the initial condition is continuous. A uniform background field $B_x=B_0$ threads the whole domain. 
